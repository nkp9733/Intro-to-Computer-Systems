...
void test(int index) {
	if (!chopsticks_available[index] || !chopsticks_available[(index + 1) % N]) { // if not both chopsticks are available
		pthread_cond_wait(&cond[index], &lock);    // wait for its neighbors		test(index);
	}
}
...
void *philosopher(void * param) {
	...
	while (1) {
		...
		printf("Philosopher %d is hungry.\n", i);
		sleep(2);
		pthread_cond_wait(&cond[i], &lock); //0wait for neighbors to eat before
